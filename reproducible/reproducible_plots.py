...
def case1and2(SAVE_PLOTS=False):
    ...
    plt.title(r"KPCA of the "f"{V}"r" dimensional dataset with DEP-CON Kernel", fontsize=18)
    ...
    plt.title(r'Linear PCA of the {} dimensional dataset'.format(V), fontsize=20)
...
# cluster counts
"""
def case4(FILENAME, SAVE_PLOTS=False):
    with open(os.path.join(__location__, '{}.json'.format(FILENAME))) as f:
        data = json.load(f)
    ...
    cluster_pred_count = {method: {i: 0 for i in range(sl, su)} for method in method_names}

    for method_name in method_names:
        scores = np.array(data["cluster_predictions"][method_name])
        for clus in range(sl,su):
            cluster_pred_count[method_name][clus] = np.count_nonzero(scores==clus)


    print("--- {}RUNS ---".format(number_of_runs))
    for method_name, verbose_name in zip(method_names, method_names_verbose):
        print("({})Number of correct cluster predictions: ".format(verbose_name), cluster_pred_count[method_name])
        print("---")
    print("---")
    ...
    for method_name in method_names:
        ...
        for i in range(sl,su):
            clus_temp+=[cluster_pred_count[method_name][i]]
        bars[method_name] = clus_temp
    ...
    for i, row in enumerate(ax):
        for j, col in enumerate(row):
            col.bar(range(sl,su), bars[method_names[2*i+j]], width=barWidth, color=color[2*i+j], alpha=0.7, edgecolor='black', linewidth=1.5, align='center', label=method_names_verbose[2*i+j])
            col.legend(loc='upper right', fontsize=14)
            col.set_ylim(top=30)
            col.set_xticks(range(sl,su))
            col.tick_params(axis='both', which='major', labelsize=16)
    fig.suptitle(r"Correct \# cluster predictions over 100 randomized runs.""\n"r" \textbf{Linear Case. True cluster count=6}", fontsize=22)
    fig.supxlabel(r"\textbf{Cluster Search Range} $\longrightarrow$", fontsize=18)
...
def case5(F1, F2, SAVE_PLOTS=False):
    ...
    plt.bar(x-barWidth/2, bar, width=barWidth, color='r', alpha=0.7, edgecolor='black', linewidth=1.5, yerr=yerr, capsize=7, align='center', label=r"\textbf{Linear Dataset}")
    ...
    plt.bar(x+barWidth/2, bar, width=barWidth, color='b', alpha=0.7, edgecolor='black', linewidth=1.5, yerr=yerr, capsize=7, align='center', label=r"\textbf{Lin. Corr. Zero Dataset}")
